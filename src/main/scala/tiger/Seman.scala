...
import tiger.Abs._
import tiger.Env.{FuncEntry, VarEntry, _}
import tiger.Types.{RECORD, _}
...
object Seman extends Seman {
  ...
  def transDec(varsEnv: venv, typesEnv: tenv, level: translate.Level, dec: Dec): (venv, tenv, List[translate.Expression]) = position(dec) match {
    ...
    case VarDec(name, escape, ty, init, position) => {
      ...
      (varsEnv + (name -> VarEntry(access, varDec)), typesEnv, List(exp))
    }
    ...
    case FunctionDecs(decs) => {
      ...
      def functionTrans(f: FunctionDec) = {
        ...
        (f.name, FuncEntry(newlevel, fname, params, result, extern = false))
      }
      ...
      def createParamsEnv(f:FunctionDec) = {
        ...
        def paramTrans(f:Field) {
          ...
          (f.name, VarEntry(access, transTy(f.ty)))
        }
